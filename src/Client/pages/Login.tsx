...
const Login: React.FC<IProps> = ({ loginfunction }) => {
  ...
  useEffect(() => {
    document.title = "User Login - Afleet"
  })
  return showLoading ?
      ...
      (<IonPage>
        <IonContent>
          <IonGrid className="outerBox">
            <IonCard className="LoginBox">
              <IonGrid>
                ...
                <IonRow className="LoginEmail">Email address*</IonRow>
                <IonRow className="LoginInput">
                  <IonInput placeholder="Enter Email Address" value={email} onIonChange={(event) => { setEmail(event.detail.value == undefined ? "" : event.detail.value?.toString()) }}></IonInput>
                </IonRow>
                <IonRow className="LoginEmail">Password*</IonRow>
                <IonRow className="LoginInput">
                  <IonInput placeholder="Enter Password" value={password} type="password" onIonChange={(event) => { setPassword(event.detail.value == undefined ? "" : event.detail.value?.toString()) }}></IonInput>
                </IonRow>
                <IonRow class="forgotPasswordButton2"><IonLabel onClick={() => { setForgotPassword(true) }}>Forgot Password</IonLabel></IonRow>
                <IonRow>
                  <IonButton className="LoginButton" onClick={async () => {
                    setShowLoading(true);
          ...
          <IonGrid>
            ...
            <IonAlert isOpen={showAlert} message="Incorrect password and username combination" onDidDismiss={() => { setEmail(""); setPassword(""); }} />
          </IonGrid>
        </IonContent>
      </IonPage>);
};

...
import CampaignDetails from "../../Ambassador/Models/CampaignDetails";
import ProgramManagementService from "../../Ambassador/Services/ProgramManagementService";
...
const Calendars: React.FC<CalendarsProps> = ({
    ...
    useEffect(() => {
        document.title = "User Calendar - Afleet"
        GetCampaignslist(range_start, range_end);
    }, []);
    const GetCampaignslist = (range_start: any, range_end: any) => {
        ClientProgramService.GetCampaignsListCalendar(loginMetadata, moment(range_start).format("YYYY-MM-DD").toString(), moment(range_end).format("YYYY-MM-DD").toString())
            .then((resp: any) => {
                console.log(resp)
                if (resp != null) {

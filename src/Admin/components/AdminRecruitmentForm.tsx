...
import { PreRequirementData } from "../../Client/Models/RequireFormData";
import AdminRecruitmentService from "../Services/AdminRecruitmentService";


interface AdminRecruitmentFormProp {
...
const AdminRecruitmentForm: React.FC<AdminRecruitmentFormProp> = ({ loginMetadata, loginfunction, newForm, setFormList, setCreateForm, form, setNewForm }) => {
  ...
  const onSubmitData = () => {
    ...
    AdminRecruitmentService.editForm(loginMetadata, JSON.stringify(fromContent), form.id)
      .then((resp) => {
        setNewForm(false);
        console.log(resp)
        AdminRecruitmentService.getAllFormData(loginMetadata)
          .then((resp) => {
            setFormList(resp);
  ...
  const onLoad = () => {
    ...
    if (newForm == true) {
      ...
      return new Promise<any>(async (resolve, reject) => {
        ...
        console.log("-------FORM DATA----------")
        // console.log(reqFormData);
        await AdminRecruitmentService.getFormDataOnLoad(loginMetadata, form.id).then((resp) => {
          console.log(resp);
          resp.map((fields: any) => {
